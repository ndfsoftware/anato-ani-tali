    ...
    effect(() => {
      ...
      if (!course) {
        this.seoService.setPageSeo({
          title: 'Curso no encontrado | Anatom�a con Ani y Tali',
          description: 'El curso solicitado no est� disponible.',
      ...
      this.seoService.setPageSeo({
        title: `${course.title} | Curso de Anatom�a para Medicina UBA`,
        description,
        path: routePath,
        type: 'article',
        image,
      ...
      this.seoService.setJsonLd(`course-${course.slug}`, {
        ...
        about: ['Anatom�a', 'Facultad de Medicina', 'UBA', 'Locomotor', 'Esplacnolog�a'],
        hasCourseInstance: {
          '@type': 'CourseInstance',
